...
export class NativeEventsReceiver {
  ...
  public registerRemoteNotificationOpened(
    callback: (
    ...
    return this.emitter.addListener(
      ...
      (response, completion) => {
        const action = response.action
          ? new NotificationActionResponse(response.action)
          : undefined;
        callback(new Notification(response), completion, action);
      }
    );
  }

  public registerRemoteNotificationsRegistrationFailed(
    callback: (event: RegistrationError) => void
  ): EmitterSubscription {
    return this.emitter.addListener(
